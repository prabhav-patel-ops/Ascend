...
import { generateSystem, previewSystem, buildTemplates, buildProgression, buildGates, TARGET_DAY_XP } from "../src/generate.js";
import { DOMAINS, ANCHORS, habitOf } from "../src/catalog.js";

/* One representative setup, used wherever the suite needs a board. */
export const SAMPLE_ANSWERS = {
  name: "Test",
  domains: ["study", "fitness"],
  goals: {
    study: { text: "Pass the exam", date: "2026-12-01" },
    fitness: { text: "Bench bodyweight", date: "" },
  },
...
const SYSTEM = generateSystem(SAMPLE_ANSWERS, "2026-09-07");
const DEFAULT_TEMPLATES = SYSTEM.templates;
const DEFAULT_PROGRESSION = SYSTEM.progression;
const SEED_GATES = SYSTEM.gates;
globalThis.SYSTEM = SYSTEM;
globalThis.SAMPLE_ANSWERS = SAMPLE_ANSWERS;
...
t("rank boundaries land on the right letters", () => {
  ...
  assert.equal(rankFromLevel(7), "D");
  assert.equal(rankFromLevel(12), "D");
  assert.equal(rankFromLevel(13), "C");
  assert.equal(rankFromLevel(19), "B");
  assert.equal(rankFromLevel(26), "A");
  assert.equal(rankFromLevel(38), "S");
  assert.equal(rankFromLevel(400), "S");
});

t("nextRankAt points upward and stops at S", () => {
  assert.equal(nextRankAt(1).rank, "D");
  assert.equal(nextRankAt(20).rank, "A");
...
t("a weekday-only habit is absent at the weekend", () => {
  ...
  assert.ok(mon.quests.some((q) => q.key === "study-second"), "the weekday block is on Monday");
  assert.ok(!sun.quests.some((q) => q.key === "study-second"), "and not on Sunday");
  assert.ok(sun.quests.length > 0, "the weekend still has a board");
});

t("progression values are interpolated into quest detail", () => {
  const day = generateDay("2026-09-01", DEFAULT_TEMPLATES, { studyPages: 21, studyProblems: 7 }, null);
  assert.match(day.quests.find((q) => q.key === "study-main").detail, /21/);
  assert.match(day.quests.find((q) => q.key === "study-problems").detail, /7/);
});
...
t("a missed quest not in today's template carries over at 1.5x", () => {
  // A weekday-only block missed on Friday is not on Saturday's board.
  const fri = generateDay("2026-09-04", DEFAULT_TEMPLATES, DEFAULT_PROGRESSION, null);
  const virt = fri.quests.find((q) => q.key === "study-second");
  fri.quests.forEach((q) => (q.done = q.key !== "study-second"));
  const sun = generateDay("2026-09-05", DEFAULT_TEMPLATES, DEFAULT_PROGRESSION, fri);
  const carried = sun.quests.find((q) => q.key === "study-second");
  assert.ok(carried, "the missed block carried over");
...
t("a missed quest already in today's template is not duplicated", () => {
  ...
  const daily = tue.quests.filter((q) => q.key === "train");
  assert.equal(daily.length, 1, "a daily habit appears once, not twice");
  assert.equal(daily[0].penalty, false);
});

t("excused quests do not carry over and are not penalised", () => {
  const mon = generateDay("2026-08-31", DEFAULT_TEMPLATES, DEFAULT_PROGRESSION, null);
  mon.quests.forEach((q) => { q.done = true; });
  const v = mon.quests.find((q) => q.key === "train");
  v.done = false;
  v.excused = true;
  const owed = dayPenalty(mon, 1).byStat;
  assert.equal(owed.STR, 0, "excused quest owes nothing");
});
...
t("an abandoned day is charged across every stat that was left short", () => {
  ...
  assert.ok(charge.byStat.STR > 0 && charge.byStat.INT > 0 && charge.byStat.VIT > 0);
...
t("miss flags raise at three in the trailing week", () => {
  const days = buildDays("2026-08-25", 7, (d, i) => {
    d.quests.forEach((q) => (q.done = !(q.stat === "INT" && i < 3)));
  });
  const flags = missFlags(days, "2026-09-01");
  assert.equal(flags.INT.flagged, true);
...
/* ---- regression: dynamic detail lines must survive serialisation ---- */
{
  ...
  t("resolveDetail fills placeholders from progression", () => {
    const out = resolveDetail("{studyPages} pages, {studyProblems} problems", { studyPages: 12, studyProblems: 3 });
    assert.equal(out, "12 pages, 3 problems");
  });
  ...
  t("a generated board survives a JSON round-trip with its tokens intact", () => {
    const cloned = JSON.parse(JSON.stringify(SYSTEM.templates));
    const main = cloned[1].find((q) => q.key === "study-main");
    assert.ok(main.detail.includes("{studyPages}"), "the token is stored, not the resolved value");
  });

  t("a generated board resolves its own starting numbers", () => {
    const day = generateDay("2026-08-31", SYSTEM.templates, SYSTEM.progression, null);
    const find = (k) => day.quests.find((q) => q.key === k);
    assert.match(find("study-main").detail, /12 pages/, "the habit's own start value");
    assert.match(find("study-problems").detail, /3 problems/);
...
/* ---- discipline model ---- */
{
  ...
  /* A board has to be generated before there is anything to grade. */
  const st = { ...emptyState(), ...gen.generateSystem(SAMPLE_ANSWERS, "2026-08-01") };
  ...
  t("leaving one quest undone still costs xp", () => {
    const d = E.generateDay("2026-08-10", st.templates, st.progression, null);
    d.quests.forEach((q, i) => (q.done = i !== 0));
    const p = E.dayPenalty(d, 1);
    assert.ok(p.total > 0, "one miss must be charged");
  });

  t("less work means a steeper charge", () => {
    const light = E.dayPenalty(mk("2026-08-10", 0.8), 1).total;
    const heavy = E.dayPenalty(mk("2026-08-10", 0.2), 1).total;
...
/* ---------- the generator ----------
   This is the part that makes the app general, so it carries the invariants
   that keep one person's system comparable to another's. */

const answersWith = (habits, extra = {}) => ({
  name: "T", domains: ["study", "fitness"], goals: {},
  habits, wake: "06:00", sleep: "22:30", trainSplit: "none",
  targets: {}, difficulty: "normal", ...extra,
});

t("everyone's day is worth about the same, however much they pick", () => {
  const few = buildTemplates(answersWith(["wake", "sleep", "study-main", "train"]));
  const many = buildTemplates(answersWith([
    "wake", "sleep", "review", "study-main", "study-problems", "study-second",
    "study-revise", "train", "cardio", "mobility", "steps",
  ]));

  const total = (list) => list.reduce((a, q) => a + q.xp, 0);
  const a = total(few[1]);
  const b = total(many[1]);

  assert.ok(Math.abs(a - TARGET_DAY_XP) < TARGET_DAY_XP * 0.2, `a small board landed at ${a}`);
  assert.ok(Math.abs(b - TARGET_DAY_XP) < TARGET_DAY_XP * 0.2, `a large board landed at ${b}`);
  assert.ok(Math.abs(a - b) < TARGET_DAY_XP * 0.25,
    `picking more boxes must not buy a bigger day (${a} vs ${b})`);
  assert.ok(many[1].length > few[1].length, "it is still a longer board, just not a richer one");
});

t("relative weight survives the scaling", () => {
  const day = buildTemplates(answersWith(["wake", "sleep", "study-main", "study-revise"]))[1];
  const main = day.find((q) => q.key === "study-main");
  const revise = day.find((q) => q.key === "study-revise");
  assert.ok(main.xp > revise.xp, "an hour block still outranks a revision pass");
});

t("cadence decides which days a habit lands on", () => {
  const t2 = buildTemplates(answersWith(["study-main", "study-second", "study-deep"]));
  const on = (wd, key) => t2[wd].some((q) => q.key === key);
  [1, 2, 3, 4, 5].forEach((wd) => assert.ok(on(wd, "study-second"), `weekday habit on ${wd}`));
  [0, 6].forEach((wd) => assert.ok(!on(wd, "study-second"), `not at the weekend (${wd})`));
  [0, 6].forEach((wd) => assert.ok(on(wd, "study-deep"), `weekend habit on ${wd}`));
  assert.ok(on(1, "study-main") && on(0, "study-main"), "a daily habit is on every day");
});

t("only scalable habits ratchet, and only one per stat", () => {
  const { progression, ratchetRules } = buildProgression(
    answersWith(["study-main", "study-problems", "cardio", "mobility"])
  );
  assert.equal(progression.studyPages, 12, "seeded from the habit's own start");
  assert.ok(progression.mobilityMins === undefined, "a habit with no scale adds nothing");

  const stats = ratchetRules.map((r) => r.stat);
  assert.equal(new Set(stats).size, stats.length,
    "two rules on one stat would mean the second never fires");
  ratchetRules.forEach((r) => {
    assert.ok(r.field && r.step > 0 && r.cap > r.step, `${r.field} is a usable rule`);
  });
});

t("a dated goal becomes a gate; an undated one stays a direction", () => {
  const gates = buildGates(answersWith(["study-main"], {
    domains: ["study", "fitness"],
    goals: { study: { text: "Pass in June", date: "2027-06-01" }, fitness: { text: "Get stronger" } },
  }), "2026-09-07");

  assert.equal(gates.length, 1, "only the dated goal becomes a deadline");
  assert.equal(gates[0].name, "Pass in June");
  assert.equal(gates[0].cleared, false);
  assert.ok(gates[0].date && gates[0].rank, "a gate needs both to be usable");
});

t("gate rank scales with how far out the deadline sits", () => {
  const at = (date) => buildGates(answersWith([], {
    domains: ["study"], goals: { study: { text: "g", date } },
  }), "2026-09-07")[0].rank;
  const near = at("2026-09-20");
  const far = at("2028-09-07");
  assert.notEqual(near, far, "a next-week deadline is not the same weight as a two-year one");
  assert.equal(far, "S");
});

t("a generated board is something the engine can actually run", () => {
  const sys = generateSystem(SAMPLE_ANSWERS, "2026-09-07");
  for (let wd = 0; wd <= 6; wd++) {
    assert.ok(Array.isArray(sys.templates[wd]), `day ${wd} has a board`);
    sys.templates[wd].forEach((q) => {
      assert.ok(q.key && q.title && q.stat && q.xp > 0 && q.slot > 0, `${q.key} is complete`);
      assert.ok(STATS.includes(q.stat), `${q.key} trains a real stat`);
    });
    const keys = sys.templates[wd].map((q) => q.key);
    assert.equal(new Set(keys).size, keys.length, `day ${wd} has no duplicate keys`);
  }
  const day = generateDay("2026-09-07", sys.templates, sys.progression, null);
  assert.ok(day.quests.length > 0);
  assert.ok(dayAvailableXp(day) > 0, "and the day is worth something");
});

t("picking no training leaves rest days rather than an empty screen", () => {
  const sys = generateSystem(answersWith(["study-main"], { trainSplit: "none" }), "2026-09-07");
  for (let wd = 0; wd <= 6; wd++) {
    assert.ok(sys.splits === undefined || true);
    assert.ok(sys.training[wd], `day ${wd} still has something`);
    assert.equal(sys.training[wd].name, "Rest");
  }
});

t("a chosen split fills every training day with real exercises", () => {
  const sys = generateSystem(answersWith(["train"], { trainSplit: "ppl" }), "2026-09-07");
  const training = Object.values(sys.training).filter((d) => d.name !== "Rest");
  assert.ok(training.length >= 3, "a six day split has training days");
  training.forEach((d) => {
    assert.ok(d.exercises.length > 0, `${d.name} has exercises`);
    d.exercises.forEach((e) => {
      assert.ok(e.key && e.name && e.repRange, `${e.name} is loggable`);
      assert.deepEqual(e.start, {}, "no load is invented — the first session sets it");
    });
  });
});

t("the review step reports what will actually be built", () => {
  const preview = previewSystem(SAMPLE_ANSWERS);
  const built = buildTemplates(SAMPLE_ANSWERS)[1];
  assert.equal(preview.count, built.length, "the count shown is the count generated");
  assert.equal(preview.perDay, built.reduce((a, q) => a + q.xp, 0), "and so is the total");
});

t("an empty answer set produces an empty board rather than throwing", () => {
  const sys = generateSystem({ habits: [], domains: [] }, "2026-09-07");
  assert.deepEqual(sys.templates[1], []);
  assert.deepEqual(sys.gates, []);
  assert.deepEqual(sys.ratchetRules, []);
  assert.ok(sys.targets.kcal > 0, "targets still fall back to something sane");
});
