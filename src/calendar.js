...
function uid(tag) {
  seq += 1;
  return `ascend-${tag}-${Date.now().toString(36)}-${seq}@local`;
}
...
export function wrapCalendar(eventLines, name = "ASCEND") {
  return [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//ASCEND//Hunter System//EN",
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH",
...
export function dailyRemindersIcs(startKey, reminders) {
  ...
  return wrapCalendar(events, "ASCEND — daily");
}
...
export function gatesIcs(gates) {
  ...
  return wrapCalendar(events, "ASCEND — gates");
}
...
export function scheduleIcs(dateKeyStr, blocks) {
  ...
  return wrapCalendar(events, `ASCEND — ${dateKeyStr}`);
}
