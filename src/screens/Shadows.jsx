...
import { STATS, STAT_META, DIFFICULTY } from "../engine.js";
import { SHADOW_PRESETS } from "../data.js";
import { exportJson, importJson } from "../store.js";
import { saveFile } from "../download.js";

export default function Shadows({ state, onExtract, onDismiss, onSettings, onImport, onReset, onProgression, onSignOut }) {
  ...
  const download = async () => {
    const name = `ascend-backup-${new Date().toISOString().slice(0, 10)}.json`;
  ...
  return (
    <>
      ...
      <Win title="Difficulty">
        <p className="faint" style={{ marginTop: 0 }}>
          The habits you chose that carry a number get harder on their own. Each steps up after
          {" "}{state.settings.ratchetStreak} clean runs in a row — raise the streak to slow it down.
        </p>
        <label className="field">
        ...
        {(state.ratchetRules || []).length === 0 && (
          <div className="empty">Nothing you picked climbs on its own. That is fine — the board stays as you set it.</div>
        )}
        {(state.ratchetRules || []).map((rule) => (
          <div className="kv" key={rule.field}>
            <span style={{ color: STAT_META[rule.stat].color }}>{rule.label || rule.field}</span>
            <b>
      ...
      <Win title="Feel">
        <p className="faint" style={{ marginTop: 0 }}>
          None of this changes what a day is worth or what anything costs. It is only how the board answers you.
        </p>
        <label className="field">
          <span>Effects</span>
          <select
            value={state.settings.fx || "full"}
            onChange={(e) => onSettings({ fx: e.target.value })}
          >
            <option value="full">Full — particles, drift, shake</option>
            <option value="lite">Lite — answers only, nothing idle</option>
            <option value="off">Off — still board</option>
          </select>
        </label>
        <label className="field">
          <span>Haptics</span>
          <select
            value={state.settings.hapticsOn === false ? "off" : "on"}
            onChange={(e) => onSettings({ hapticsOn: e.target.value === "on" })}
          >
            <option value="on">On</option>
            <option value="off">Off</option>
          </select>
        ...
        <p className="faint" style={{ marginBottom: 0 }}>
          Lite drops the drifting background, the screen shake and every idle loop, and halves the particle count — it
          is the setting for an old phone or a long day off the charger. If your phone is set to reduce motion, that
          wins over whatever is chosen here. Haptics are silent on iPhone, which gives web pages no vibration at all.
        </p>
      </Win>
      ...
      <Win title="Profile" right={state.hunter?.name}>
        <p className="faint" style={{ marginTop: 0 }}>
          Signed in as <b>{state.hunter?.name}</b> on this device. Signing out closes this board and returns to the
          profile list — it changes nothing and deletes nothing. Everything here is still here when you come back.
        </p>
        <p className="faint">
          There is no password, so this is not a lock. Anyone using this browser can open any profile on the list. It
          is here so a shared device can hold more than one board.
        </p>
        <button className="ghost" style={{ width: "100%" }} onClick={onSignOut}>
          Sign out
        </button>
      </Win>
