...
import {
  STAT_META, dayAvailableXp, dayEarnedXp, isDayCleared,
...
export default function Quests({ state, todayKey, onToggle, onExcuse, onAddExtra, onRemoveExtra }) {
  ...
  const grade = gradeDay(day);
  const esc = escalationAt(state.days, viewKey);
  const charge = dayPenalty(day, esc.multiplier);
  ...
  return (
    <>
      ...
      {isToday && owedTotal > 0 && (
        <Win title="At stake" tone="danger" right={`${charge.days} days`}>
          <p className="faint" style={{ marginTop: 0 }}>
            If the day ended now it would cost {owedTotal} xp — {charge.days} days of work
            {esc.multiplier > 1 && ` at ${esc.multiplier.toFixed(2)}× escalation`}. Still winnable.
          </p>
          {esc.effective > 0 && (
            <p className="faint">
      ...
      {!isToday && !isFuture && owedTotal > 0 && (
        <Win title="Charged" tone="danger" right={`−${charge.days} days`}>
          <p className="faint" style={{ marginTop: 0 }}>
            Fell {Math.round(grade.shortfall * 100)}% short at {esc.multiplier.toFixed(2)}× escalation.
