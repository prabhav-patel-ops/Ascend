...
export const STAT_META = {
  STR: { label: "Strength", feeds: "Training and physical output", color: "#FF7A4A" },
  AGI: { label: "Agility", feeds: "Execution and thinking under pressure", color: "#3FE0A8" },
  INT: { label: "Intelligence", feeds: "Study and depth", color: "#A98BFF" },
  PER: { label: "Perception", feeds: "Craft and judgement", color: "#FFC24B" },
...
// Calibrated against a simulated year: diligent play reaches D in a month,
// C by three months, B by six, A at a year. S stays genuinely out of reach.
const RANK_TABLE = [
  { min: 38, rank: "S" },
  { min: 26, rank: "A" },
  { min: 19, rank: "B" },
 ...
 */
export function generateDay(dk, templatesByWeekday, prog, prevDay) {
  ...
  return { date: dk, quests, generated: true, levelUpSeen: false };
}
...
export function isDayCleared(day, mode) {
  ...
  const bar = (DIFFICULTY[mode] || {}).clearBar ?? CLEAR_THRESHOLD;
...
// A day counts as a lapse below this ratio.
 ...
 */
export function gradeDay(day) {
  const avail = dayChargeableXp(day);
  if (avail === 0) return { ratio: 1, shortfall: 0, grade: "void" };
  const earned = day.quests.filter((q) => q.done && !q.excused).reduce((s, q) => s + q.xp, 0);
  const ratio = Math.min(1, earned / avail);
  const shortfall = Math.max(0, CLEAN_BAR - ratio);

  let grade;
  if (shortfall === 0) grade = "clean";
  else if (ratio >= LAPSE_BAR) grade = "held";
  else if (ratio >= 0.4) grade = "slipped";
  else grade = "broken";

  return { ratio, shortfall, grade };
}
 ...
 */
export function escalationAt(days, dk, todayKey) {
  ...
  for (let i = run + 1; i <= LAPSE_WINDOW; i++) {
    const d = days[shiftKey(dk, -i)];
    if (!d || !d.generated) break;
    if (gradeDay(d).grade === "clean") cleanRun += 1;
    else break;
  }
 ...
 */
export function dayPenalty(day, multiplier = 1, depth = PENALTY_DEPTH) {
  const out = {};
  STATS.forEach((s) => (out[s] = 0));
  const { shortfall } = gradeDay(day);
...
export function projectedPenalty(days, dk) {
  const day = days[dk];
  if (!day) return null;
  const esc = escalationAt(days, dk);
  return { ...dayPenalty(day, esc.multiplier), escalation: esc };
}
...
/* Rules are generated per person from the habits they chose — see
   generate.js. This empty default means a system with no scalable habit
   simply never ratchets, rather than ratcheting someone else's numbers. */
 ...
 */
export function applyRatchet(days, endKey, prog, settings = {}, rules = RATCHET_RULES) {
  ...
  list.forEach((rule) => {
    ...
    // At most one step per field per day. Without this, every save after a
    // clean run would raise the bar again and the number would run away.
    if (stamped[rule.field] === endKey) return;
    ...
    for (let i = 0; i < 60; i++) {
      ...
      if (stamped[rule.field] && k <= stamped[rule.field]) break;
      const d = days[k];
      if (!d) continue;
    ...
    if (run >= streakNeeded && next[rule.field] < rule.cap) {
      const was = next[rule.field];
      next[rule.field] = Math.min(rule.cap, was + rule.step);
      if (next[rule.field] !== was) {
        stamped[rule.field] = endKey;
        raised.push({ stat, field: rule.field, from: was, to: next[rule.field], unit: rule.unit, label: rule.label });
      }
    }
  });
...
export function recomputeTotals(days, shadows, todayKey, mode) {
  ...
  keys.forEach((k) => {
    ...
    const esc = escalationAt(days, k, todayKey);
    const pen = dayPenalty(d, esc.multiplier, rules.depth);
...
export function monthCells(year, month, days = {}, todayKey, mode) {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    const key = dateKey(d);
    const day = days[key];
    const graded = day && day.generated ? gradeDay(day) : null;
    cells.push({
      key,
...
export function goalProgress(goal, state, season, todayKey) {
  ...
  switch (goal.type) {
    case "cleanDays":
      value = keys.filter((k) => days[k].generated && gradeDay(days[k]).grade === "clean").length;
      break;
    case "clearedDays":
